...
parse_config(Element = #xmlElement{name = radius, attributes = Attrs},
	...
	SessionData = case {RadType, ElementType} of
		{acc, _} ->
			...
			Duration = getAttr(integer, Element#xmlElement.content, accounting, duration),
			Interval = getAttr(integer, Element#xmlElement.content, accounting, interval),
			CbMod = getAttr(atom, Element#xmlElement.content, accounting, cb_mod),
			DefParams#radius_request{cb_mod = CbMod, port = Port, 
					interim = Interim, duration = Duration, interval = Interval,
